...
public class TrackworkBlocks {
    ...
    public static final BlockEntry<WheelBlock> MED_SIMPLE_WHEEL =
            REGISTRATE.block("med_simple_wheel", p -> new WheelBlock(p, TrackworkBlockEntityTypes.MED_SIMPLE_WHEEL))
                    .initialProperties(() -> Blocks.RAIL)
                    .properties(p -> p.mapColor(MapColor.BROWN).noCollision().strength(7.0f).sounds(BlockSoundGroup.METAL))
                    .transform(BlockStressDefaults.setNoImpact())
                    .transform(pickaxeOnly())
    ...
    public static final BlockEntry<? extends RotatedPillarKineticBlock> SIMPLE_WHEEL_PART = wheelpart("simple_wheel_part");
    public static final BlockEntry<? extends RotatedPillarKineticBlock> MED_SIMPLE_WHEEL_PART = wheelpart("med_simple_wheel_part");

    public static BlockEntry<? extends RotatedPillarKineticBlock> wheelpart(String name) {
        return REGISTRATE.block(name, (properties) -> new RotatedPillarKineticBlock(properties) {
                    @Override
